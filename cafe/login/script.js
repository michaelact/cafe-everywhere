document.getElementById("loginForm").addEventListener("submit", function (event) {
    ...
    const apiEndpoint = "http://localhost:9999/cafe/login";

    // Check if cafe is already authenticated
    const cafeId = getCookie("cafeid");
    if (cafeId) {
        // Redirect to menu page if cafe is already logged in
        ...
        .then(data => {
            // Mock redirection to order page and set cafe cookie (replace this with your actual redirection and cookie logic)
            if (data.status === "Success") {
                setCookie("cafeid", data.data.id, 1); // Set cookie for 30 days
